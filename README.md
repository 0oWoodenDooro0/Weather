...
這是一個 Android 天氣預報應用程式，利用 **Jetpack Compose** 構建使用者介面，並遵循 **MVVM (Model-View-ViewModel)**
架構模式開發。應用程式能根據使用者目前的 **GPS 位置**，透過整合外部天氣 **API**，顯示**即時天氣資訊**及**未來數天的天氣預測
**。
...
* **未來天氣預報：** 提供未來 [請填寫天數，例如：五天] 的天氣預測數據。
...
* **非同步處理：** Kotlin Coroutines / Flow (如果您的 API 請求使用了這些)
* **網路請求：** Retrofit
